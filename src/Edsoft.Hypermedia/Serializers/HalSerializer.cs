...
namespace Edsoft.Hypermedia.Serializers
{
    public class HalSerializer : JsonSerializer
    {
        ...
        public override string Serialize(HypermediaRepresentation representor)
        {
            if (representor == null)
            {
        ...
        private JObject CreateJObjectForRepresentor(HypermediaRepresentation representor)
        {
            var jObject = SerializeAttributesToJObject(representor);

            if (!String.IsNullOrWhiteSpace(representor.SelfLink)) AddLinkFromTransition(jObject, new HypermediaTransition { Rel = "self", Uri = representor.SelfLink });
        ...
        private void AddRepresentorsToEmbedded(IList<HypermediaRepresentation> list, JObject embeddedJObject, string embeddedResourceKey)
        {
            if (list.Count == 1)
            {
        ...
        private void AddLinkFromTransition(JObject document, HypermediaTransition transition)
        {
        ...
        protected virtual JObject CreateLinkObjectFromTransition(HypermediaTransition transition)
        {
        ...
        public override IRepresentationBuilder DeserializeToNewBuilder(string message, Func<IRepresentationBuilder> builderFactoryMethod)
        {
            if (message == null)
            {
        ...
        private IRepresentationBuilder BuildRepresentorBuilderFromJObject(Func<IRepresentationBuilder> builderFactoryMethod, JObject document)
        {
        ...
        private void CreateEmbeddedResources(JObject document, IRepresentationBuilder currentBuilder,
            Func<IRepresentationBuilder> builderFactoryMethod)
        {
        ...
        private IEnumerable<HypermediaRepresentation> GetRepresentorsFromEmbeddedJObject(Func<IRepresentationBuilder> builderFactoryMethod, JToken propertyJToken)
        {
            var representorsInCollection = new List<HypermediaRepresentation>();
        ...
        private static void SetSelfLinkIfPresent(JObject document, IRepresentationBuilder builder)
        {
        ...
        private void CreateTransitions(JObject document, IRepresentationBuilder builder)
        {
            if (document["_links"] == null) return;
        ...
        protected virtual HypermediaTransition GetTransitionFromLinkObject(JToken link, string rel)
        {
            var href = link["href"];
            ...
            var transition = new HypermediaTransition
            {
                Rel = rel,

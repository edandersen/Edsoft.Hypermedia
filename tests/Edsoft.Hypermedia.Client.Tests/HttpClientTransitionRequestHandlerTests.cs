...
namespace Edsoft.Hypermedia.Tests
{
    public class HttpClientTransitionRequestHandlerTests : TestWithFixture
    {
        ...
        public async Task RequestTransitionAsync_CallsSendAsyncWithRequestMessageForSimpleTransition()
        {
            ...
            var transition = new HypermediaTransition { Uri = relativeUri };
            ...
            serializer.Stub(
                s =>
                    s.DeserializeToNewBuilder(Arg<string>.Is.Equal(representorAsJson),
                        Arg<Func<IRepresentationBuilder>>.Matches(m => m().GetType() == typeof(RepresentationBuilder))))
        ...
        public async Task RequestTransitionAsync_SupportsCustomHttpMethodInTransition()
        {
            ...
            var transition = new HypermediaTransition { Uri = relativeUri, Methods = new []{httpMethod} };
            var representorResult = Fixture.Create<HypermediaRepresentation>();
            var representorAsJson = Fixture.Create<string>();
            var representorBuilder = MockRepository.GenerateMock<IRepresentationBuilder>();
            representorBuilder.Stub(r => r.ToRepresention()).Return(representorResult);
            ...
            serializer.Stub(
                s =>
                    s.DeserializeToNewBuilder(Arg<string>.Is.Equal(representorAsJson),
                        Arg<Func<IRepresentationBuilder>>.Matches(m => m().GetType() == typeof(RepresentationBuilder)))).Return(representorBuilder);
